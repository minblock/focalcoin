...
const char * const BITCOIN_CONF_FILENAME = "focalcoin.conf";
...
#else
    const char* pszModule = "focalcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Focalcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Focalcoin";
#else
    // Unix
    return pathRet / ".focalcoin";
#endif
#endif
}
